...
function keyPressed() {
  ...
  if (state === "cardcam" && key === " ") {
    ...
    capturedImage = createImage(1000, 600);
    capturedImage.copy(
      capture,
      0,
      0,
...
function Stage3readMission() {
  ...
  if (!speechReady && speech === "") {
    // 음성 인식 대기 상태
    imageMode(CENTER);
    image(mic, width/2, height/2, 300, 130);
...
async function displayQRCode() {
  background(200);
  image(capturedImage, 100, 100, 35, 350);

  if (!qrCodeLoded) {
    noLoop();
    const graphics = createGraphics(500, 300);
    graphics.image(capturedImage, 0, 0, 500, 300);

    const canvas = document.createElement("canvas");
    canvas.width = 500;
    canvas.height = 300;
    const ctx = canvas.getContext("2d");
    ctx.drawImage(graphics.canvas, 0, 0, 500, 300);
    ...
    bodyData.append("image", blob, "image.png");

    let response = await fetch("https://api.imgur.com/3/image", {
      method: "POST",
      headers: {
        Authorization: "Client-ID e1339f13a577e3d",
  ...
  fill(0);
  textAlign(CENTER);
  text("사진을 저장하시려면 QR을 스캔해주시고, 스페이스 바를 눌러 다음 단계로 이동하세요.", width / 2, height - 50);
}
